//! user sha512sum — print or check 512-bit checksums.
use std::io;
...
pub fn run() -> i32 {
 ...
 "Usage: sha512sum [OPTION]... [FILE]...\n Print or check 512-bit checksums.\n -c, --check read checksums from the FILEs and check them\n With no FILE, or when FILE is -, read standard input.\n"
            ...
            "--version" => {
                println!("sha512sum (user_utils) 0.1.0");
                return 0;
            }
            "-c" | "--check" => check = true,
            s if s.starts_with('-') && s != "-" => {
                eprintln!("sha512sum: invalid option -- '{s}'");
                return 1;
            }
...
fn hash_file(path: &str) -> io::Result<String> {
    let mut h = usercore::digest::Sha512::new();
    ...
    for list in files {
        ...
        for line in data.lines() {
            ...
            let (hash, file) = if let Some((h, f)) = line.split_once(" ") {
                ...
                eprintln!("sha512sum: invalid line");
                status = 1;
                continue;
            };
