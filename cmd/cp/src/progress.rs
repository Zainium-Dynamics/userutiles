// progress.rs — Progress rendering for --progress copies.
//
// Receives ProgressEvents over a crossbeam channel and drives an indicatif
// MultiProgress display following the Zainium cyber-tech aesthetic:
//
...
/// Returns `(files_copied, total_bytes)` when the channel is closed.
pub fn render_progress(rx: Receiver<ProgressEvent>) -> (u64, u64) {
    ...
    overall.set_style(
        ProgressStyle::with_template(
            "\x1b[96m cp\x1b[0m \x1b[35m{bytes}\x1b[0m/\x1b[35m{total_bytes}\x1b[0m \
        ...
        match event {
            ...
            ProgressEvent::FileDone { path, bytes } => {
                if let Some(pb) = file_bars.remove(&path) {
                    ...
                    // Skipped/instant file (e.g. reflink) — keep overall bar in sync.
    ...
    overall.finish_with_message(format!(
        "\x1b[92m✓ Done\x1b[0m \x1b[35m{files_done} file(s) copied\x1b[0m \
         \x1b[33m{}\x1b[0m",
        ByteSize(bytes_done)
    ));
