...
fn tmp_dir(tag: &str) -> PathBuf {
 ...
 .join(format!("xmv_test_{tag}_{}", std::process::id()));
...
fn test_crossdev_move_file() {
 ...
 assert_eq!(fs::read(&dest_file).unwrap(), data, "content must be intact");
 fs::remove_dir_all(&dir).ok();
}
...
fn test_undo_move() {
 ...
 let mut j = journal(&dir);
 let offset = j.record(Operation::Move {
 src: src.clone(),
 dest: dest.clone(),
 }).unwrap();
...
fn test_undo_move_performed_via_crossdev_engine() {
 ...
 assert_eq!(fs::read(&src).unwrap(), data, "content must survive the round trip");
 fs::remove_dir_all(&dir).ok();
}
...
fn test_trash_move() {
 ...
 let info = trash_path.with_extension("txt.trashinfo")
 .parent().unwrap()
 .parent().unwrap()
 .join("info")
 .join(format!("{}.trashinfo", trash_path.file_name().unwrap().to_string_lossy()));
