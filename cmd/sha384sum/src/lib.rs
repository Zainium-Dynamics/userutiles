//! user sha384sum — print or check 384-bit checksums.
use std::io;
...
pub fn run() -> i32 {
    ...
    for arg in std::env::args().skip(1) {
        match arg.as_str() {
            "-h" | "--help" => {
                print!(
 "Usage: sha384sum [OPTION]... [FILE]...\n Print or check 384-bit checksums.\n -c, --check read checksums from the FILEs and check them\n With no FILE, or when FILE is -, read standard input.\n"
 );
                return 0;
            }
            "--version" => {
                println!("sha384sum (user_utils) 0.1.0");
                return 0;
            }
            "-c" | "--check" => check = true,
            s if s.starts_with('-') && s != "-" => {
                eprintln!("sha384sum: invalid option -- '{s}'");
                return 1;
            }
...
fn hash_file(path: &str) -> io::Result<String> {
    let mut h = usercore::digest::Sha384::new();
    ...
    for list in files {
        ...
        for line in data.lines() {
            ...
            let (hash, file) = if let Some((h, f)) = line.split_once(" ") {
                ...
                eprintln!("sha384sum: invalid line");
                status = 1;
                continue;
            };
